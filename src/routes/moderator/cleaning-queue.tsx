...
export const Route = createFileRoute("/moderator/cleaning-queue")({
  loader: async () => {
    try {
  ...
  component: CleaningQueuePage,
});

function CleaningQueuePage() {
  const { items } = Route.useLoaderData();
  const { user } = Route.useRouteContext();

  const visibleItems = user?.role === "admin"
    ? items
    : items.filter((it) => it.assigned_to === user?.id);
      ...
      <Card>
        <CardContent className="pt-6 divide-y divide-border">
          {visibleItems.length === 0 ? (
            <p className="text-sm text-muted-foreground py-3">No items in your cleaning queue.</p>
          ) : (
            visibleItems.map((it) => (
