...
export const APIRoute = createAPIFileRoute("/api/threads/$id/messages")({
  POST: async ({ request, params }) => {
    ...
    }>("SELECT user_id, vendor_id, messaging_blocked FROM vendor_threads WHERE id = $1", [params.id]);
    ...
    if (isVendor && vendorMember && !(vendorMember as { can_respond_messages: boolean }).can_respond_messages) {
