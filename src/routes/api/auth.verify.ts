...
export const APIRoute = createAPIFileRoute("/api/auth/verify")({
  GET: async ({ request }) => {
    try {
      ...
      if (tokens.length === 0) {
        return Response.json({ success: false, error: "Invalid verification token" }, { status: 404 });
      }

      const tokenRecord = tokens[0];
      if (tokenRecord.used_at) {
        return Response.json({ success: false, error: "This verification token has already been used" }, { status: 400 });
      }
      if (new Date(tokenRecord.expires_at) < new Date()) {
        return Response.json({ success: false, error: "Verification token has expired" }, { status: 400 });
      }

      await query(`UPDATE verification_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = $1`, [tokenRecord.id]);
